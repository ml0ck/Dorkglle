...
def build_dork(base_term, keywords, filetypes, search_type):
    keyword_part = " OR ".join([f'"{kw.strip()}"' for kw in keywords if kw.strip()])
    filetype_part = ' OR '.join([f'filetype:{ft.strip()}' for ft in filetypes if ft.strip()])
    ...
    dork_types = {
        ...
        19: f"{base_term} filetype:pdf ({keyword_part})",
        20: f"{base_term} inurl:(/admin OR /backup) ({keyword_part})",
        21: f"{base_term} inurl:mongodb ({keyword_part})"
    }
...
def main():
    while True:
        ...
        if choice.isdigit() and 1 <= int(choice) <= 21:
            base_term = input(Fore.LIGHTWHITE_EX + "Enter the base term for your search: ").strip()
            keywords = get_user_input(Fore.LIGHTWHITE_EX + "Enter keywords (separated by commas): ")
            filetypes = get_user_input(Fore.LIGHTWHITE_EX + "Enter file types (separated by commas): ")
